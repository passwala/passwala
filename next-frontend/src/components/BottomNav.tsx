  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-background border-t pb-safe pt-2 px-2 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
      <div className="flex items-center justify-around">
        ...
        <NavItem href="/events" icon={Calendar} label="Events" isActive={pathname.startsWith('/events')} />
        <NavItem href="/sports" icon={LayoutGrid} label="Sports" isActive={pathname.startsWith('/sports')} />
        <NavItem href="/orders" icon={Ticket} label="Orders" isActive={pathname.startsWith('/orders')} />
        <NavItem href="/profile" icon={User} label="Profile" isActive={pathname.startsWith('/profile')} />
      </div>
    </div>
  );
}
